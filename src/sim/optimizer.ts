...
// Finds which openings to open so wind flows THROUGH the rooms instead of
// dying inside. Exhaustive for small numbers of toggleable openings,
// greedy + local-search refinement for larger plans. Runs in async chunks so
// the UI stays responsive.

import { Plan, OptimizerResult } from '../model/types';
import { solve, scoreField } from './solver';

const OPT_ITERATIONS = 200; // coarser solve for what-if runs (less noise than 160)

function evaluate(plan: Plan, openIds: Set<string>): { score: number; coverage: number; meanSpeed: number } {
  const f = solve(plan, { iterations: OPT_ITERATIONS, openIds });
  const s = scoreField(f, plan.wind.speed);
  return { score: s.score, coverage: s.coverage, meanSpeed: s.meanSpeed };
}
...
export async function optimize(
  ...
  const consider = (openIds: string[], s: { score: number; coverage: number; meanSpeed: number }) => {
    if (!best || s.score > best.score) {
      best = { openIds, score: s.score, coverage: s.coverage, meanSpeed: s.meanSpeed };
    }
  };

  if (free.length <= 10) {
    // Exhaustive: up to 1024 coarse solves.
    ...
    for (let mask = 1; mask < total; mask++) {
      ...
      if (mask % 16 === 0) { onProgress?.({ done: mask, total, best }); await yield_(); }
    }
